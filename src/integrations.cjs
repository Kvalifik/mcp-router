...
function createIntegrations({home,appData,executable,script,socket,run,applicationsDirs=['/Applications',path.join(home,'Applications')],binaryDirs=[path.join(home,'.local/bin'),'/opt/homebrew/bin','/usr/local/bin',...(process.env.PATH||'').split(path.delimiter).filter(Boolean)]}) {
  ...
  const appPath=name=>first(applicationsDirs.map(dir=>path.join(dir,name+'.app')));
  const binary=name=>first(binaryDirs.map(dir=>path.join(dir,name)));
  function discover() {
    const codexApp=appPath('Codex')||appPath('ChatGPT');
    const codex=first([...(codexApp?[path.join(codexApp,'Contents/Resources/codex')]:[]),...(binary('codex')?[binary('codex')]:[])]);
    return [
      {id:'codex',name:'ChatGPT/Codex',command:codex,installed:!!codex,download:'https://openai.com/codex/',hint:'Reload MCP connections or start a new Codex task.'},
  ...
  async function status(c) {
    ...
    try {
      ...
      if(c.id==='codex') {
        try {const {stdout}=await run(c.command,['mcp','get',NAME,'--json'],{timeout:10000,cwd:home});const d=JSON.parse(stdout);existing=d.transport;enabled=d.enabled!==false;}
  ...
  async function connect(id) {
    ...
    try {
      if(id==='codex')await run(c.command,['mcp','add',NAME,'--env','ELECTRON_RUN_AS_NODE=1','--env',`ROUTER_SOCKET=${socket}`,'--',executable,script],{timeout:15000,cwd:home});
