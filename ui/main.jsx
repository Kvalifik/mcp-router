...
async function checkForUpdates(manual=false) {
  ...
  if(result.state==='available') {
    ...
    toast[result.state==='error'?'error':'info'](message,{id:'app-update'});
  }
}
function AboutPanel() {
...
function PermissionForm({ model, data, busy, perform, close, connect, refresh }) {
  ...
  const canTest = item.enabled && item.read && data.connections.some(c => c.id === item.connectionId && c.enabled);
  ...
  return <form className="dialog-form" onSubmit={submit}>
    ...
    {!defaults&&<div className="flex items-center justify-between gap-2"><Label>MCP server</Label><DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline" size="sm">{channel==='inherit'?`Follow default (${selectedChannel})`:channel==='beta'?'Beta':'Stable'}<ChevronDown className="size-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuRadioGroup value={channel} onValueChange={selectChannel}>{[['inherit','Follow default'],['stable','Stable'],['beta','Beta']].map(([value,label])=><DropdownMenuRadioItem key={value} value={value}>{label}{!hasAccess(value==='inherit'?(connection?.channel||'stable'):value)&&<span className="ml-auto pl-3 text-xs text-muted-foreground">Authorize</span>}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu></div>}
    {!defaults&&requestedChannel!==null&&<div role="status" className="rounded-md border bg-background p-3 space-y-2"><p className="text-sm font-medium">{authorizing?'Waiting for authorization…':`Authorize ${requestedEndpoint==='beta'?'Beta':'Stable'} for ${item.name}`}</p><p className="text-xs text-muted-foreground">{authorizing?'Complete the Webflow prompt in your browser, then return here. Your edits are kept.':'Select this project in Webflow’s authorization screen. Include any other projects that should keep access through this connection. Your current server stays unchanged until you save.'}</p><div className="flex gap-2"><Button type="button" size="sm" disabled={busy||!connection?.enabled} onClick={authorizeRequested}>{authorizing?'Open authorization again':`Authorize ${requestedEndpoint==='beta'?'Beta':'Stable'}`}</Button><Button type="button" size="sm" variant="ghost" disabled={busy} onClick={()=>{setRequestedChannel(null);setAuthorizing(false);}}>Cancel</Button></div>{!connection?.enabled&&<p className="text-xs text-muted-foreground">Enable this connection first.</p>}</div>}
    ...
    <DialogFooter className="shrink-0 border-t pt-4">{!defaults && <Button type="button" variant="outline" aria-label="Test connection" title={canTest?'Test connection using saved permissions':'Testing requires an enabled project and connection, with saved Site read permission'} disabled={busy || !canTest} onClick={async()=>{try{await perform('/api/read-project',{projectId:item.id});toast.success(`${item.name}: read succeeded.`);}catch(e){toast.error(e.message);}}}>Test connection</Button>}<Button type="button" variant="outline" disabled={busy} onClick={close}>Cancel</Button><Button disabled={busy || requestedChannel!==null || !defaults && !name.trim()}>{busy?'Saving…':'Save changes'}</Button></DialogFooter>
  </form>;
}
...
function ClientConnections() {
  ...
  const load = async()=>setClients(await window.routerDesktop.clientStatus());
  ...
  return <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm" disabled={!!pending} aria-label="App connections"><AppWindow className="size-4" aria-hidden="true" />Apps<ChevronDown className="size-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" sideOffset={20} className="w-64">{clients.filter(c=>c.installed).map(c=><DropdownMenuItem key={c.id} className="group gap-3 py-2.5" disabled={!c.installed||!!pending} aria-label={`${c.state==='configured'||c.state==='needs-reconnect'?'Reconnect':'Connect'} ${c.name}`} title={c.state==='configured'?c.hint:c.error || ''} onSelect={async event=>{event.preventDefault();setPending(c.id);const notice=toast.loading(`Connecting ${c.name}…`);try{const result=await window.routerDesktop.connectClient(c.id);if(!result.ok)throw new Error(result.error);toast.success(result.message,{id:notice,duration:10000});await load();}catch(e){toast.error(e.message,{id:notice,duration:10000});}finally{setPending(null);}}}><ClientIcon id={c.id} /><span className="min-w-0 flex-1 truncate">{c.name}</span>{pending===c.id?<LoaderCircle className="size-3.5 animate-spin" aria-label="Connecting" />:!c.installed?<span className="text-xs text-muted-foreground">Not installed</span>:c.state==='configured'?<span className="size-3.5" title="Configured · Click to reconnect"><Check className="size-3.5 text-emerald-600 group-hover:hidden group-focus:hidden" /><RefreshCw className="hidden size-3.5 group-hover:block group-focus:block" /></span>:c.state==='needs-reconnect'||c.state==='error'?<RefreshCw className="size-3.5" />:<Plus className="size-3.5" />}</DropdownMenuItem>)}<DropdownMenuSeparator />{clients.some(c=>!c.installed)&&<DropdownMenuSub><DropdownMenuSubTrigger><Plus className="mr-2 size-4" />Add another app…</DropdownMenuSubTrigger><DropdownMenuSubContent>{clients.filter(c=>!c.installed).map(c=><DropdownMenuItem key={c.id} className="gap-3" title={`Download ${c.name}`} onSelect={async()=>{try{await window.routerDesktop.clientDownload(c.id);}catch{toast.error('Could not open the download page.');}}}><ClientIcon id={c.id} /><span className="flex-1">{c.name}</span><ArrowUpRight className="size-3.5" /></DropdownMenuItem>)}</DropdownMenuSubContent></DropdownMenuSub>}<DropdownMenuSub><DropdownMenuSubTrigger><Copy className="mr-2 size-4" />Copy configuration</DropdownMenuSubTrigger><DropdownMenuSubContent>{[['standard','Standard MCP'],['vscode','VS Code']].map(([format,label])=><DropdownMenuItem key={format} onSelect={async event=>{event.preventDefault();setCopied(null);try{await window.routerDesktop.copyClientConfig(format);setCopied(format);toast.success(`${label} configuration copied to clipboard.`,{duration:6000});}catch{toast.error('Could not copy configuration.',{duration:6000});}}}>{copied===format?<Check className="size-4 text-emerald-600" />:<Copy className="size-4" />}{label}{copied===format&&<span role="status" className="ml-auto text-xs text-muted-foreground">Copied</span>}</DropdownMenuItem>)}</DropdownMenuSubContent></DropdownMenuSub></DropdownMenuContent></DropdownMenu>;
...
function GrantComparison({connection,onReconnect,busy}) {
  ...
  const {missingProjects,missingScopes,missingWorkspaceIds}=comparison;
  const mismatch=missingProjects.length||missingScopes?.length||missingWorkspaceIds?.length;
  return <div className="rounded-md border bg-background p-3 space-y-2 text-xs"><p className="font-medium text-sm">{mismatch?'Beta access differs from Stable':'Project access matches Stable'}</p>
    ...
    {!!missingScopes?.length&&<p>Missing OAuth permissions: {missingScopes.join(', ')}</p>}
    {missingScopes===null&&<p className="text-muted-foreground">Webflow did not report both OAuth scope lists. Permission parity is unverified.</p>}
    ...
    {!!mismatch&&<><p className="text-muted-foreground">If this subset is intentional, keep it. Otherwise reconnect Beta and include the missing projects and permissions.</p><Button type="button" size="sm" variant="outline" disabled={busy} onClick={onReconnect}>Fix Beta access</Button></>}
  </div>;
}
function NameForm({ model, busy, perform, close }) {
  ...
  async function submit(e) {
    e.preventDefault();
    try { await perform(`/api/${model.kind}/${item.id}/edit`, useDefault ? {useDefaultName:true} : {name}); toast.success('Display name saved.'); close(); }
    catch(e) { toast.error(e.message); }
  }
  return <form onSubmit={submit} className="space-y-4">
    ...
    <DialogFooter><Button type="button" variant="outline" disabled={busy} onClick={close}>Cancel</Button><Button disabled={busy}>Save changes</Button></DialogFooter>
  </form>;
}
function App() {
  ...
  useEffect(() => {load().catch(() => {setBootFailed(true);toast.error('Cannot connect to the router. Reopen the app.');});const timer=setInterval(() => {if(!document.hidden&&!modalRef.current)load().catch(()=>{});},15000);return()=>clearInterval(timer);},[]);
  ...
  useEffect(()=>{
    for(const c of data?.connections||[]){const result=c.grantComparison;if(!result)continue;
      ...
      if(result.missingProjects.length||result.missingScopes?.length||result.missingWorkspaceIds?.length)toast.warning(`${c.name}: Beta access differs from Stable.`,{id:`beta-grant-${c.id}`,duration:10000,action:{label:'Review',onClick:()=>{if(modalRef.current){toast.info('Save or cancel your project edits, then review MCP server access from the connection menu.');}else setModal({type:'channel',item:c});}}});
    }
  },[data]);
  ...
  return <main className="router-main">
        ...
        <div className="flex flex-wrap items-center gap-2 p-3">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label={`More options for ${c.name}`} title={`More options for ${c.name}`} disabled={busy}><Ellipsis className="size-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onSelect={()=>setModal({type:'channel',item:c})}><Server className="size-4" />MCP server</DropdownMenuItem>
    ...
    <Dialog open={!!modal} onOpenChange={open=>{if(!open&&!busy)setModal(null);}}><DialogContent onInteractOutside={event=>{if(event.target.closest('[data-sonner-toaster]'))event.preventDefault();}} onOpenAutoFocus={event=>{if(['permissions','rename'].includes(modal?.type)){event.preventDefault();event.target.focus();}}} className={`dialog-shell ${['permissions','defaults'].includes(modal?.type)?'permission-dialog sm:max-w-2xl':'sm:max-w-lg'}`}>{modal?.type!=='permissions'&&<DialogHeader className="shrink-0 pr-6"><DialogTitle>{modal&&titles[modal.type]}</DialogTitle><DialogDescription className={['permissions','about'].includes(modal?.type)?'sr-only':undefined}>{modal?.type==='permissions'?'Edit the project name and permissions.':modal?.type==='about'?'Application information and third-party licenses.':modal?.type==='defaults'?'Starting permissions for automatically discovered projects.':modal?.type==='delete'?`Remove ${modal.item.name} from this router?`:modal?.type==='channel'?'Authorize each server for this connection. Choose the default in Settings.':modal?.type==='order'?'Drag connections or use the arrows to change their order.':modal?.type==='diagnostics'?'Connection checks and recent activity.':'Change the local display name.'}</DialogDescription></DialogHeader>}
      ...
      {modal?.type==='channel'&&<div className="space-y-4">{['stable','beta'].map(channel=>{const c=data.connections.find(c=>c.id===modal.item.id),state=c?.channels?.[channel];return <div key={channel} className="flex items-center justify-between gap-3 rounded-md border bg-background p-3"><div><Label className="capitalize">{channel}</Label><p className="text-xs text-muted-foreground">{state?.authorized?`${state.siteIds?.length||0} projects authorized`:'Authorization required'}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" disabled={busy||!c?.enabled} onClick={()=>action(async()=>{await connect(c.id,channel);setModal(null);},'Complete OAuth in your browser.')}>{state?.authorized?'Reconnect':'Authorize'}</Button></div></div>;})}<GrantComparison connection={data.connections.find(c=>c.id===modal.item.id)} busy={busy} onReconnect={()=>action(async()=>{await connect(modal.item.id,'beta');setModal(null);},'Complete Beta authorization in your browser.')} /><p className="text-xs text-muted-foreground">The default is set in Settings. Beta uses the existing permission catalog; new or changed tools are not automatically enabled.</p><DialogFooter><Button variant="outline" onClick={()=>setModal(null)}>Done</Button></DialogFooter></div>}
    {modal?.type==='order'&&<ConnectionOrderForm connections={data.connections} busy={busy} perform={perform} close={()=>setModal(null)} />}
      {modal?.type==='rename'&&<NameForm key={modal.item.id} model={modal} busy={busy} perform={perform} close={()=>setModal(null)} />}
